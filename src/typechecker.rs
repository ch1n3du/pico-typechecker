...
//     bin_ops: HashMap<Token, HashSet<(String, String)>>,
// }
...
//             bin_ops: crate::lookup::get_bin_ops(),
//         }
//     }
...
//                 let var_tipo = self.get_var_tipo(name)?;
//                 let val_tipo = self.check_expr(&value)?;

//                 if var_tipo != val_tipo {
//                     Err(TypeError::Basic(format!(
//                         "Variable {name} of type {var_tipo:?} can't be reassigned to {val_tipo:?}."
//                     )))
//                 } else {
...
//                     (None, None) => return Err(TypeError::Basic(
//                         "If not initializer is given the type of the variable must be declared."
//                             .to_string(),
//                     )),
//                     (Some(t), None) => t.clone(),
//                     (None, Some(e)) => self.check_expr(e)?,
//                     (Some(expected_tipo), Some(initializer)) => {
...
//                 let condition_tipo = self.check_expr(condition)?;
//                 if condition_tipo.name != "bool" {
//                     return Err(TypeError::Basic(format!(
//                         "The condition of 'while' statement must be a boolean expression."
//                     )));
//                 }
...
//                 falsy_branch,
//             } => {
//                 if self.check_expr(predicate)?.name == "bool" {
...
//     #[test]
//     fn dummy() {
//         let lhs: Box<Expr> = Box::new(Expr::Value(Value::Number(12.0)));
//         let rhs: Box<Expr> = Box::new(Expr::Value(Value::Number(18.0)));
